...
The **Const Type Args** framework provides numerous libraries, samples, and documentation to support working with *const type arguments*. Const type arguments, or **const type args**, are used to effectively "pass" compile-time-like values to generic types & methods via generic parameters and accessible in static contexts such as type initialization, and static methods, fields, &amp; properties.
...
//
    ///// <summary>
    ///// A const-type-arg struct that provides a delegate to square an integer.
    ///// </summary>
    //public readonly struct SquareIntDelegate : IConstTypeArg<Func<int, int>>
    //{
        //// The strongly-typed delegate: takes an int, returns an int (the squared value).
        //public static Func<int, int> Value => x => x * x;
    //}
////
